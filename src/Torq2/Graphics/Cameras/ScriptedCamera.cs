...
namespace Torq2.Graphics.Cameras
{
	...
	public class ScriptedCamera : Camera
	{
		...
		{
			m_tPosition = m_pCurvePosition.GetPointOnCurve((float) gameTime.ElapsedGameTime.TotalMilliseconds);
			m_tLookAt = m_pCurveLookAt.GetPointOnCurve((float)gameTime.ElapsedGameTime.TotalMilliseconds);
